...
const int TARGET = 11;
const int NUMBER_INDIVIDUAL = 10;
...
vector<int> mutate_parent(vector<int>);
...
void genetic_algorithm(const int size)
{
    ...
    while (result != TARGET)
    {
        population.push_back((generate_percent(1) > .5) ? cross_over(population[index], population[index - 1]) : mutate_parent(population[index]));
...
vector<int> mutate_parent(vector<int> parent)
{
